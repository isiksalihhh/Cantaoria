...
(function($) {
    ...
    function getImageThumbnailHtml(src) {
        return '<img src="' + src + '" alt="Image preview" class="thumbnail d-block " style="border:1px solid gray; border-radius:20px; max-width: ' + options.maxWidth + 'px; max-height: ' + options.maxHeight + 'px ">';
    }
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'Dosya boyutu çok yüksek (en fazla ' + options.maxFileSizeKb + 'kB) olmalýdýr.');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Hatalý görsel.');
        }
        else {
            callback(false, 'Dosya tipi desteklenmiyor.');
        }
    }
    ...
    function isValidImageUrl(url, callback) {
        ...
        image.onerror = function() {
            if (!timeout) {
                window.clearTimeout(timer);
                callback(false, 'Görsel bulunamadý.');
            }
        };
        ...
        timer = window.setTimeout(function() {
            ...
            callback(false, 'Görsel yüklenirken zaman aþýmýna uðradý.');
        }, timeoutMs);
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn i').addClass('fa-solid fa-pen');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('i').addClass('fa-solid fa-pen');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onerror = function() {
                    $fileTab.prepend(getAlertHtml('Görsel yüklenirken hata oluþtu.'));
                    $fileInput.val('');
                };
